...
YoloOnnx::YoloOnnx(const std::string& model_path, const int input_width,
                   const int input_height, const float conf_thres,
                   const float iou_thres, int ort_threads)
...
std::vector<Detection> YoloOnnx::infer(const cv::Mat& bgr_image) {
    ...
    Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
        ...
        input_tensor_shape_.data(), input_tensor_shape_.size());
...
YoloOnnx::LetterboxInfo YoloOnnx::preprocessToInput(
    ...
    const int resized_width =
        static_cast<int>(std::round(original_width * scale));
    const int resized_height =
        static_cast<int>(std::round(original_height * scale));
    const int pad_x = (input_width_ - resized_width) / 2;
    const int pad_y = (input_height_ - resized_height) / 2;

    cv::Mat resized;
    cv::resize(bgr_image, resized, cv::Size(resized_width, resized_height), 0.0,
               0.0, cv::INTER_LINEAR);

    cv::Mat letterboxed(input_height_, input_width_, CV_8UC3,
                        cv::Scalar(114, 114, 114));
    resized.copyTo(
        letterboxed(cv::Rect(pad_x, pad_y, resized_width, resized_height)));
    ...
    for (int y = 0; y < input_height_; ++y) {
        ...
        for (int x = 0; x < input_width_; ++x) {
            ...
            chw_buffer[area + index] =
                static_cast<float>(pixel[1]) * scale_norm;
            chw_buffer[2 * area + index] =
                static_cast<float>(pixel[2]) * scale_norm;
        }
    }
...
std::vector<Detection> YoloOnnx::decodeOutput(
    const float* output_data, const std::vector<int64_t>& shape,
    ...
    if (stride != kStrideNoObj && stride != kStrideObj) {
        throw std::runtime_error(
            "Unsupported YOLO output stride, expected 84 or 85.");
    }
    ...
    auto valueAt = [&](const int64_t pred_index, const int channel) -> float {
        if (channels_first) {
            return output_data[static_cast<size_t>(channel) *
                                   static_cast<size_t>(num_preds) +
                               pred_index];
        }
        return output_data[pred_index * static_cast<size_t>(stride) +
    ...
    for (int64_t i = 0; i < num_preds; ++i) {
        ...
        const float x1 = (cx - 0.5F * w - static_cast<float>(letterbox.pad_x)) /
                         letterbox.scale;
        const float y1 = (cy - 0.5F * h - static_cast<float>(letterbox.pad_y)) /
                         letterbox.scale;
        const float x2 = (cx + 0.5F * w - static_cast<float>(letterbox.pad_x)) /
                         letterbox.scale;
        const float y2 = (cy + 0.5F * h - static_cast<float>(letterbox.pad_y)) /
                         letterbox.scale;

        const int left = clampInt(static_cast<int>(std::floor(x1)), 0,
                                  letterbox.original_width - 1);
        const int top = clampInt(static_cast<int>(std::floor(y1)), 0,
                                 letterbox.original_height - 1);
        const int right = clampInt(static_cast<int>(std::ceil(x2)), 0,
                                   letterbox.original_width - 1);
        const int bottom = clampInt(static_cast<int>(std::ceil(y2)), 0,
                                    letterbox.original_height - 1);
