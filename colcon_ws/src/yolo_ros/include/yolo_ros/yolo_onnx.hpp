...
public:
  YoloOnnx(
    const std::string & model_path, int input_width, int input_height, float conf_thres,
    float iou_thres, int ort_threads);
...
private:
  ...
  LetterboxInfo preprocessToInput(const cv::Mat & bgr_image, std::vector<float> & chw_buffer) const;
